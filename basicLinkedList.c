// return ile listeyi dondurerek temel bagli liste islemlerinin yapilmasi
...
struct node* insert2 (struct node* head, int key, int order)
{
...
struct node* insert3 (struct node* head, int key)
{
	struct node* temp1 = create(key);
	if(head==NULL)
	{
...
struct node* delete3 (struct node* head,int order)
{
	struct node* temp1 = head;
